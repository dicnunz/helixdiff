...
| Latest candidate oracle | 4 same-seed unseen validation gaps: morphology/factor lattice contains the exact hidden span in `4/4` cases; fixed structural prior ranks it top-4 in `4/4` and selects it top-1 in `1/4`; this is reranker proof only, not selected model accuracy |
