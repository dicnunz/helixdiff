...
Status: live research packet for the 3-hour loop started on 2026-05-21. Current continuation status: the Chrome-backed ChatGPT teammate route is live through the extension browser-client; the HelixDiff ChatGPT tab is open in Extended Pro at `https://chatgpt.com/c/6a0f1cf7-b734-83ea-afa1-1a92152682f1`, and the latest readback recommends moving off proxy calibration into a redacted in-document echo lattice. The local proxy failure remains sharp: generic visible pseudo masks are not target-shaped enough. This continuation adds six cheap proof ratchets: blanked/swapped visible-context counterfactuals must reduce selected-exact, a proxy-mask selector contract must be chosen from visible-only pseudo holes before target scoring, the proxy receipt must fail a stricter `--require-useful-ratchet` gate before anyone can cite it as target-lift evidence, `proxy_geometry_mode: target_retrieval` must fail closed unless target-geometry-shaped proxies beat shuffle on pseudo heldout, `proxy_geometry_mode: target_shadow` must fail closed unless label-free redacted target-lattice fingerprints transfer to pseudo heldout without prior harm, and redacted in-document echo must prove baseline-preserving K=128 containment lift without target/anchor-window overlap.
