...
- **Retrieval-lattice diffusion scoring.** The benchmark can expose top visible suture candidates plus bridge/unigram proposals, score them with leave-one-out denoiser probes, and select by diffusion score plus boundary, morphology, and surface priors. It reports selected accuracy, oracle coverage, and whether a fixed structural prior placed the exact answer in the reranker-sized top-k set.
