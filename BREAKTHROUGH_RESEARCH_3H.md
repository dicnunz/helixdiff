...
Status: live research packet for the 3-hour loop started on 2026-05-21. The Chrome-backed ChatGPT teammate route is now usable through the Codex Chrome bridge, and the live ChatGPT thread `https://chatgpt.com/c/6a0f1cf7-b734-83ea-afa1-1a92152682f1` was run in `Extended Pro`. The useful critique was narrow and severe: before spending more Mac heat on model scoring, emit a model-free visible-reranker oracle smoke proving `prior`, `surface`, and `visible_reranker` branches are present, the visible reranker is calibration-only with `apply=false`, leakage checks are logged, and shuffle falsification is reported.
