...
Status: live research packet for the 3-hour loop started on 2026-05-21. Current continuation status: the Chrome-backed ChatGPT teammate route is live through the extension browser-client; the HelixDiff ChatGPT tab is open in Extended Pro at `https://chatgpt.com/c/6a0f1cf7-b734-83ea-afa1-1a92152682f1`. The latest readback recommended converting redacted echo containment into a causal echo-dominance selector with blank and swapped-edge nulls. The local result is deliberately negative: redacted echo still improves K=128 containment, but direct promotion does not improve top-4 selection, so the selector contract is killed fail-closed and target-lift claims are blocked. The next real bottleneck is verifier/reranker selection, not wider candidate soup.
...
Extended Pro's newest cheap move after target-shadow failed closed is **Redacted In-Document Echo Lattice**: first replace the hidden target bytes with sentinel bytes, forbid the target span and local anchor window, then mine exact bi-anchor, left/right echo, and morphology echo candidates only from the still-visible bytes of the same eval document. The landed smoke `proof/in_document_echo_oracle_smoke.json` is model-free and passed on 8 seeded validation gaps: prior K=128 containment `75.0%`, baseline-preserving prior+echo K=128 containment `100.0%`, echo-alone containment `75.0%`, shuffled echo containment `15.625%`, and zero target/anchor/sentinel overlap hits. This is transductive visible-document candidate containment, not model accuracy.

Extended Pro's follow-up was **Causal Echo-Dominance Selector Contract**: promote a redacted echo candidate only if real same-document anchors dominate blank and swapped-edge nulls, freeze the contract before target scoring, and fail closed if promotion cannot beat the prior top-4 without harm. The landed smoke `proof/echo_dominance_selector_contract_smoke.json` passes as a negative proof: `model_load=false`, target bytes unavailable to the selector, zero target/anchor overlap hits, promoted cases `7/8`, blank/swapped null promotions `0`, but top-4 exact stays tied at `37.5%` while selected exact falls from `37.5%` to `25.0%`. The emitted contract is `ready_for_heldout=false`, `apply=false`, and `diagnostic_only=true`. That kills direct echo promotion on this slice; echo remains useful as a containment source for future verifier scoring, not as an applied selector claim.
...
That does not prove accuracy. It proves the bottleneck moved. Before this patch, the runner could not select missing answers. After this patch, the next hard problem is selecting/verifying the right candidate without laundering a hand-built lattice into a model-quality claim.

Source refresh, 2026-05-21: D3PM showed that the discrete corruption transition is a first-class design choice, including absorbing states that connect diffusion to masked and autoregressive modeling (`https://arxiv.org/abs/2107.03006`). SEDD showed that ratio/score-style objectives can make discrete diffusion language models competitive and useful for controllable infilling (`https://arxiv.org/abs/2310.16834`). XDLM's 2026 claim is a useful warning shot: the frontier is moving toward kernels that bridge masked-diffusion understanding and uniform-noise generation rather than toward bigger left-to-right samplers alone (`https://arxiv.org/abs/2602.01362`). For HelixDiff on a Mac, the actionable translation is narrow: do not chase a huge general LM; build a tiny repair specialist whose corruption kernel, candidate lattice, and verifier calibration are all optimized for visible-context document repair.
...
- bridge-guided sampling copies bridge mistakes;
...
The selector now has a predeclaration artifact instead of only a diagnostic receipt. `helixdiff-selector-contract` consumes `helixdiff-calibrate-selector` JSON, records calibration/source hashes, freezes the chosen selector anchor and margin into benchmark flags, and marks the artifact `diagnostic_only` unless the calibration has enough cases and a candidate recommendation. That small contract matters because it blocks the most tempting false breakthrough: using the same scored cases to discover the selector and then calling the result held-out proof. A real narrow repair claim now has to move through calibration -> frozen selector contract -> separate held-out benchmark -> gate.
...
It emits the ranked lanes, source IDs, claim boundary, current ChatGPT teammate state, and exact proof commands. The current top lane is now `visible_reranker_oracle_smoke`: run `helixdiff-visible-reranker-oracle-smoke` to produce a model-free proof receipt before any heavy benchmark. The next cheap lane is `proxy_mask_selector_contract_smoke`: run `helixdiff-proxy-mask-selector-contract-smoke` to freeze a visible-only selector preset before target scoring. The next oracle lane is `gold_blind_bi_anchor_oracle`, which uses the `helixdiff-proof-recipe` oracle command and train-split-only bi-anchor gap candidates. The first heavy lane remains `strict_repair_lattice_proof`: run the predeclared `helixdiff-proof-recipe` benchmark under the shared Heavy Slot, then run its calibrate, `helixdiff-selector-contract`, and `helixdiff-gate --require-repair-proof-contract` commands. That recipe now requires the strict anchor sweep `prior,surface,visible_reranker`, diagnostic-only visible-reranker calibration, train-only bi-anchor candidate logging, and a frozen selector-contract artifact, so the next heavy run cannot ignore the newest selector mechanism or relabel in-sample selector tuning as held-out proof.

The new smoke receipt is `proof/visible_reranker_oracle_smoke.json`. It is intentionally not a model win: `model_load=false`, `visible_reranker.apply=false`, `verdict=pass`, `gold_in_lattice_at_128=1.0` on the 2-case smoke, `visible_reranker.selected_exact=1.0`, leakage flags are zero, shuffle falsification drops selected-exact to `0.333`, blank-context selected-exact drops to `0.0`, swapped-edge selected-exact drops to `0.5`, and causal-visible selected-exact stays at `1.0`. Its `proof_contract` now requires both shuffle collapse and counterfactual-context collapse before the receipt can pass, so the smoke cannot merely print controls and ignore them. That is a proof-integrity ratchet, not a selected-generation benchmark.

The proxy-mask selector receipt is `proof/proxy_mask_selector_contract_smoke.json`, with the frozen contract in `proof/proxy_mask_selector_contract_smoke_contract.json`. It is also model-free. The default contract remains boundary-only because that is the currently proven ready path: it chooses among a tiny preset space using pseudo masks cut only from visible context, holds out half of those pseudo masks as an internal falsification check, redacts target spans before calibration, and records `target_metric_used_for_selection=false`. The new target-retrieval-geometry mode is implemented as a falsification probe, not a promoted selector. It redacts the target bytes, builds a target lattice/source profile from visible context, chooses visible proxy holes whose redacted lattice geometry is closer than the pool mean, and then correctly returns `verdict=fail` on the current smoke because pseudo-heldout top-4 does not beat shuffle. The target rows remain after-freeze diagnostics; they do not license a repair claim. A new `--require-useful-ratchet` mode makes that non-claim mechanical: the current checked-in smoke has `useful_ratchet=false`, so the claim gate must fail until the contract actually improves after-freeze target rows or a separate held-out benchmark proves lift.

The newest proxy idea is target-retrieval geometry: redact the target bytes, build the target lattice, then choose visible-only proxy masks whose candidate-source geometry resembles that redacted target lattice before target scoring exists. It is a plausible Mac-local breakthrough lever because it tries to make self-supervised proxy masks match the failure geometry of the real hidden span instead of just matching punctuation and boundary shape. The current implementation is intentionally non-default for proof claims: when `proxy_geometry_mode=target_retrieval`, geometry shaping itself passes, but pseudo-heldout top-4 does not beat shuffle, so the receipt verdict fails. That is the right outcome for a strange idea that is promising but not yet evidence.

The follow-up local idea is sharper: target-shadow proxy calibration. Instead of matching only coarse source geometry, HelixDiff now can build a label-free redacted target lattice fingerprint with source diversity, selector-top disagreement, prior entropy, top-gap pressure, pairwise candidate edit-distance statistics, and duplicate-source pressure. `proxy_geometry_mode=target_shadow` oversamples visible pseudo holes, discards target-anchor-window overlaps, keeps the nearest fingerprint shadows, and freezes a selector only if shadow heldout clears the shuffle gate. Current result: the fingerprint matching works and exact target bytes are not used for matching, but shadow heldout still fails the transfer gate, so `contract_ready=false`. That is progress: the breakthrough lane now rejects a more target-shaped proxy if it is still non-transferable.
