...
Status: live research packet for the 3-hour loop started on 2026-05-21. Current continuation status: the Chrome-backed ChatGPT teammate route fails at `node_repl/js Transport closed`, so no fresh GPT-5.5 Pro contribution is claimed from this run. The useful recorded critique remains narrow and severe: before spending more Mac heat on model scoring, emit a model-free visible-reranker oracle smoke proving `prior`, `surface`, and `visible_reranker` branches are present, the visible reranker is calibration-only with `apply=false`, leakage checks are logged, and shuffle falsification is reported. This continuation adds three cheap proof ratchets: blanked/swapped visible-context counterfactuals must reduce selected-exact, a proxy-mask selector contract must be chosen from visible-only pseudo holes before target scoring, and the proxy receipt must fail a stricter `--require-useful-ratchet` gate before anyone can cite it as target-lift evidence.
