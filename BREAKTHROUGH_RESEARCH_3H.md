...
Status: live research packet for the 3-hour loop started on 2026-05-21. Current continuation status: the Chrome-backed ChatGPT teammate route is live again through the extension browser-client; the HelixDiff thread is open at `https://chatgpt.com/c/6a0f1cf7-b734-83ea-afa1-1a92152682f1` in Extended Pro. The latest useful critique targeted the real proxy failure: generic visible pseudo masks are not target-shaped, so the next probe should synthesize calibration tasks from the target's redacted retrieval geometry. This continuation adds four cheap proof ratchets: blanked/swapped visible-context counterfactuals must reduce selected-exact, a proxy-mask selector contract must be chosen from visible-only pseudo holes before target scoring, the proxy receipt must fail a stricter `--require-useful-ratchet` gate before anyone can cite it as target-lift evidence, and the new `proxy_geometry_mode: target_retrieval` probe must fail closed unless target-geometry-shaped proxies beat shuffle on pseudo heldout.
